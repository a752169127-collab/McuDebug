...
from core.test_automation import AutomationVariableSpec
...
class JLinkWorker(QObject):
    ...
    memory_block_read_done = Signal(int, bool, object, object, str)
    memory_block_write_done = Signal(int, bool, object, object, str)
    automation_snapshot_done = Signal(int, bool, object, str)
    automation_write_done = Signal(int, bool, object, str)
    ...
    def write_typed(self, request_id: int, address: int, type_name: str, value_text: str) -> None:
        try:
            ...
            # V0.5: JLINK_WriteMemEx returning the full byte count is the write
            # acknowledgement. Do not add a second ReadMemEx solely to verify.
            self._jlink.write_memory(address, data)
            value = decode_value(data, type_name)
            self.manual_write_done.emit(request_id, True, value, data.hex(" ").upper(), "")
        except Exception as exc:
            self.manual_write_done.emit(request_id, False, None, "", str(exc))

    @Slot(int, object, int)
    def read_memory_block(self, request_id: int, address: int, size: int) -> None:
        try:
            address = int(address)
            size = int(size)
            if size <= 0 or size > 4096:
                raise ValueError("Memory Explorer read size must be in range 1..4096 bytes")
            data = self._jlink.read_memory(address, size)
            self.memory_block_read_done.emit(request_id, True, address, data, "")
        except Exception as exc:
            self.memory_block_read_done.emit(request_id, False, address, b"", str(exc))

    @Slot(int, object, object)
    def write_memory_block(self, request_id: int, address: int, data) -> None:
        try:
            address = int(address)
            payload = bytes(data)
            if not payload or len(payload) > 4096:
                raise ValueError("Memory Explorer write size must be in range 1..4096 bytes")
            self._jlink.write_memory(address, payload)
            self.memory_block_write_done.emit(request_id, True, address, payload, "")
        except Exception as exc:
            self.memory_block_write_done.emit(request_id, False, address, b"", str(exc))

    # ---------- Test Automation ----------
    @Slot(int, object)
    def read_automation_snapshot(self, request_id: int, specs: list[AutomationVariableSpec]) -> None:
        """Read one no-code automation snapshot through the single J-Link owner.

        This is deliberately request/response based rather than a second sampling
        thread. The Test Automation page controls cadence and never touches the DLL.
        """
        try:
            ...
            if not variables:
                raise ValueError("Automation snapshot has no variables")
            planner = MemoryReadPlanner(max_gap_bytes=0, max_block_bytes=256)
            results: list[dict[str, Any]] = []
            values_by_id: dict[int, float | int] = {}
            for block in planner.plan(variables):
                ...
                for var in block.variables:
                    ...
                    values_by_id[int(var.row_id)] = decode_value(raw, var.type_name)
            for var in variables:
                if int(var.row_id) not in values_by_id:
                    continue
                value = values_by_id[int(var.row_id)]
                results.append({
                    "row_id": int(var.row_id),
                    "name": var.name,
            ...
            self.automation_snapshot_done.emit(request_id, True, results, "")
        except Exception as exc:
            self.automation_snapshot_done.emit(request_id, False, [], str(exc))

    @Slot(int, object, str, str)
    def write_automation_value(self, request_id: int, address: int, type_name: str, value_text: str) -> None:
        try:
            if not self._jlink.is_target_connected:
                raise RuntimeError("Target is not connected")
            address = int(address)
            ...
            self._jlink.write_memory(address, data)
            value = decode_value(data, type_name)
            self.automation_write_done.emit(
                request_id,
                True,
                {
                    "address": address,
                    "type_name": type_name,
                    "value": value,
                    "formatted": format_value(value, type_name),
                },
                "",
            )
        except Exception as exc:
            self.automation_write_done.emit(request_id, False, {}, str(exc))
