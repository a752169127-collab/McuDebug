...
class MainWindow(QMainWindow):
    ...
    memory_block_read_requested = Signal(int, object, int)
    memory_block_write_requested = Signal(int, object, object)
    ...
    automation_snapshot_requested = Signal(int, object)
    automation_write_requested = Signal(int, object, str, str)
    shutdown_worker_requested = Signal()

    def __init__(self) -> None:
        super().__init__()
        self.setWindowTitle("MCU Debug Assistant V0.6.3 - Test Automation Studio")
        ...
        self._pending_memory_reads: dict[int, tuple[int, int]] = {}
        self._pending_memory_writes: dict[int, tuple[int, bytes]] = {}
        ...
        self.memory_block_read_requested.connect(self._worker.read_memory_block)
        self.memory_block_write_requested.connect(self._worker.write_memory_block)
        ...
        self.automation_snapshot_requested.connect(self._worker.read_automation_snapshot)
        self.automation_write_requested.connect(self._worker.write_automation_value)
        ...
        self._worker.memory_block_read_done.connect(self._on_memory_block_read_done)
        self._worker.memory_block_write_done.connect(self._on_memory_block_write_done)
        ...
        self.test_automation_page.run_state_changed.connect(self._on_automation_run_state_changed)
        self.test_automation_page.log_requested.connect(self._log)
        self._worker.automation_snapshot_done.connect(self.test_automation_page.handle_snapshot_done)
        self._worker.automation_write_done.connect(self.test_automation_page.handle_write_done)
        self.tabs.addTab(self.test_automation_page, "Test Automation")
        ...
        # Restore the last selected AXF/ELF as an active symbol source, not just
        # as text in the path field. Defer until the event loop starts so the
        # complete UI/log surface already exists before parsing a potentially
        # large debug file.
        QTimer.singleShot(0, self._auto_load_saved_symbol_file)
    ...
    def _build_manual_memory_page(self) -> QWidget:
        page = QWidget()
        layout = QVBoxLayout(page)
        box = QGroupBox("Typed Memory Read / Write")
        grid = QGridLayout(box)
        ...
        self.memory_explorer = MemoryExplorerPage(self)
    ...
    def _write_watch_row(self, row_id: int) -> None:
        ...
        try:
            ...
            # WriteMemEx in the single J-Link owner thread.
    ...
    def _auto_load_saved_symbol_file(self) -> None:
        """Load the persisted AXF/ELF once at startup when it still exists."""
        saved_path = self.symbol_file_edit.text().strip()
        if not saved_path:
            return
        candidate = Path(saved_path).expanduser()
        if not candidate.is_file():
            self._log(f"AXF/ELF auto-load skipped: file not found: {saved_path}")
            return
        self._log(f"AXF/ELF auto-load: {candidate.name}")
        self._load_symbol_file(str(candidate), rebind_watch=True)
    ...
    def _load_symbol_file(self, path: str | None = None, *, rebind_watch: bool = False) -> bool:
        ...
        try:
            ...
            if hasattr(self, "memory_explorer"):
                self.memory_explorer.set_symbols(result.symbols)
            if hasattr(self, "test_automation_page"):
                self.test_automation_page.set_symbols(result.symbols)
            ...
            if hasattr(self, "memory_explorer"):
                self.memory_explorer.clear_symbols()
            if hasattr(self, "test_automation_page"):
                self.test_automation_page.clear_symbols()
    ...
    def _request_memory_block(self, address: int, size: int) -> None:
        if not self._connected:
            return
        try:
            address = int(address)
            ...
            if not 0 <= address <= 0xFFFFFFFF:
                raise ValueError("Memory address is outside 32-bit range")
            if size <= 0 or address + size > 0x100000000:
                raise ValueError("Invalid Memory Explorer read range")
            request_id = self._new_request_id("memory_block_read")
            self._pending_memory_reads[request_id] = (address, size)
            self.memory_block_read_requested.emit(request_id, address, size)
        except Exception as exc:
            self._show_error("Memory read failed", exc)

    def _write_memory_block(self, address: int, data: bytes) -> None:
        if not self._connected:
            self._show_error("Memory write failed", RuntimeError("J-Link is not connected"))
            return
        try:
            address = int(address)
            payload = bytes(data)
            if not payload:
                return
            # Memory Explorer V0.5.1 follows CE-style direct editing: keyboard
            # input/paste writes immediately, while double-click editing is
            # confirmed by the edit dialog's OK button itself. Do not add a
            # second modal confirmation here. The separate manual Memory R/W
            # form keeps its explicit confirmation behavior.
            request_id = self._new_request_id("memory_block_write")
            self._pending_memory_writes[request_id] = (address, payload)
    ...
    def _add_memory_address_to_watch(self, name: str, address: int, type_name: str) -> None:
        try:
            ...
            if type_name not in supported_types():
                type_name = "uint8"
            if self.watch_table.contains_variable(name, address):
                self._log(f"Memory -> Watch: skipped existing {name} @ 0x{address:08X}")
                return
            self.watch_table.add_variable(
                name=name,
            ...
            self._push_watch_specs_if_sampling()
            self._log(f"Memory -> Watch: added {name} @ 0x{address:08X} ({type_name})")
        except Exception as exc:
            self._show_error("Add Watch failed", exc)
        ...
        if row_id is not None:
            ...
            for spec in self.watch_table.specs():
                if spec.row_id == row_id:
                    ...
                    self._log(f"WATCH WRITE {spec.name} = {format_value(value, spec.type_name)} WRITE=OK")
                    break
        elif kind == "manual_write":
            type_name = self.type_combo.currentText()
            self.current_value_edit.setText(format_value(value, type_name))
            self.raw_value_edit.setText(raw)
            self._log(f"WRITE value={self.current_value_edit.text()} WRITE=OK")

    def _on_memory_block_read_done(self, request_id: int, ok: bool, address, data, error: str) -> None:
        self._pending_manual.pop(request_id, None)
        requested = self._pending_memory_reads.pop(request_id, None)
        if not ok:
            if requested is not None:
                self._log(
                    f"MEMORY READ ERROR @ 0x{requested[0]:08X} size={requested[1]}: {error}"
                )
            return
        if hasattr(self, "memory_explorer"):
            self.memory_explorer.set_block(int(address), bytes(data))

    def _on_memory_block_write_done(self, request_id: int, ok: bool, address, data, error: str) -> None:
        self._pending_manual.pop(request_id, None)
        pending = self._pending_memory_writes.pop(request_id, None)
        if not ok:
            self._show_error("Memory write failed", RuntimeError(error))
            return
        payload = bytes(data)
        if hasattr(self, "memory_explorer"):
            self.memory_explorer.patch_bytes(int(address), payload)
        self._log(
            f"MEMORY WRITE @ 0x{int(address):08X} size={len(payload)} "
            f"raw={payload.hex(' ').upper()} WRITE=OK"
        )
    ...
    # ---------- Test Automation ----------
    def _on_automation_run_state_changed(self, active: bool) -> None:
        # V0.6.0 uses deterministic request/response memory polling for workflow
        # steps. Stop the two continuous samplers first so automation reads are
        # not interleaved with Watch/HSS/RTT traffic on the single J-Link owner.
        if active:
            if self._sampling:
                self.stop_sampling_requested.emit()
            if self._scope_sampling:
                self.stop_scope_requested.emit()
            self._log("Test Automation: exclusive polling mode; Watch/Scope sampling stopped")
    ...
    def _update_connection_ui(self, connected: bool) -> None:
        ...
        if hasattr(self, "scope_page"):
            self.scope_page.set_connected(connected)
        if hasattr(self, "memory_explorer"):
            self.memory_explorer.set_connected(connected)
        if hasattr(self, "test_automation_page"):
            self.test_automation_page.set_connected(connected)
        for w in (
    ...
    def _load_settings(self) -> None:
        ...
        if hasattr(self, "memory_explorer"):
            memory_text = str(self._setting("memory_explorer_state", "") or "")
            if memory_text:
                try:
                    self.memory_explorer.load_settings_state(json.loads(memory_text))
                except Exception:
                    pass

        if hasattr(self, "test_automation_page"):
            automation_text = str(self._setting("test_automation_state", "") or "")
            if automation_text:
                try:
                    self.test_automation_page.load_settings_state(json.loads(automation_text))
                except Exception:
                    pass
    ...
    def _save_settings(self) -> None:
        ...
        if hasattr(self, "memory_explorer"):
            self._settings.setValue(
                "memory_explorer_state",
                json.dumps(self.memory_explorer.settings_state(), ensure_ascii=False),
            )
        if hasattr(self, "test_automation_page"):
            self._settings.setValue(
                "test_automation_state",
                json.dumps(self.test_automation_page.settings_state(), ensure_ascii=False),
            )
