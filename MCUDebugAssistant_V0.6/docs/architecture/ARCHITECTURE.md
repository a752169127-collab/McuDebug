...
## V0.6.7 Memory navigation commit architecture

```text
Address/Symbol editor
   ├─ exact Symbol → SymbolIndex → typed goto → keep Symbol text
   ├─ visible completer + highlighted row + Enter → exact selected Symbol once
   ├─ History activated(index) → MRU model row → direct navigation
   └─ numeric text → canonical raw address

Watch Set Value editor
   textEdited → dirty
      ↓
Qt delegate commit (Enter or focus-out)
      ↓
setModelData()
      ↓ dirty exactly once
write_requested → MainWindow → single JLinkWorker → WriteMemEx
```

Symbol auto-fit uses only visible-row SymbolIndex lookups + FontMetrics and a presentation cache. It does not alter Memory read planning or trigger target I/O.

## V0.6.8 Symbol completion / history activation boundary

```text
Typing / AXF candidate popup
        ↓
QCompleter.activated(Symbol)       ← only Symbol candidate commit path
        ↓
SymbolIndex exact record
        ↓
Typed Memory goto + Symbol text

History arrow / MRU popup
        ↓
NavigationHistoryComboBox popup session
        ↓
activated(int) only if that popup was opened
        ↓
selected MRU query
```

The editable combo and the Symbol completer share one line edit but do not share activation semantics. A raw Enter cannot infer a Symbol from a stale current completion, and a Symbol completion cannot cascade into an old history row. History deletion and popup width fitting are local presentation/state operations and must not emit target I/O.


## V0.6.9 History selection vs history mutation
History selection and history mutation are separate operations. Selecting a saved query resolves it locally and navigates Memory without calling the MRU insertion helper, so row order is stable. A newly typed successful query may still update the bounded MRU list. Single-entry deletion is exposed as a persistent toolbar button so it remains available after the combo popup closes; deletion only mutates local history/QSettings and preserves current navigation.


## V0.6.10 — Compact connection presentation

```text
J-Link Connection Group
  ├─ Always-visible header:  −/+ | connection summary | Disconnect
  └─ Collapsible details: DLL path / DLL / Device / Interface / Speed / Connect

Disconnected -> details expanded
Connected    -> details collapsed automatically
Manual +     -> details visible but configuration remains locked
Disconnect   -> details expanded again
```

Collapse state is presentation-only. It does not create a new Worker/session, does not reconfigure the active target, and does not emit target I/O. Automation business tables likewise hide redundant Qt row headers and use row order instead of a duplicated `#` column.


## V0.6.11 — Engineering preset parameter entry

```text
User parameter control
      ↓
editable preset combo
      ├─ choose common value
      └─ type custom in-range value
      ↓
integer value()
      ↓
existing Watch / Memory / Scope logic
```

`IntPresetComboBox` is a UI adapter, not a new timing layer. Memory auto-refresh still drives its existing QTimer; Watch still sends the selected interval to the single JLinkWorker; Scope sampling still passes requested Hz into the existing HSS start path; Scope buffer still sizes the existing preallocated ring; FPS still feeds the existing single-shot absolute-deadline presentation pacer. Settings remain integers, so old saved configurations load without migration.

Scope Buffer remains capped at 120 s. The current raw store has a 1.5M-point maximum; increasing the visible preset to 300 s without redesigning that capacity would make high-rate configurations silently retain less time than the UI promises.


## V0.6.12 — Semantic typed Memory renderer

```text
Address/Symbol navigation
        │
        ├─ Raw address ───────────────> Raw HexMemoryView
        │
        └─ AXF/DWARF Symbol ─────────> SymbolMemoryView
                                         │
                                         v
                              plan_symbol_read_window()
                                  ...
                                  ReadMemEx once
                                   ...
                                   Raw block cache
                                         │
                            SymbolIndex.scalar_starts_in_range
                                         │
                          ┌──────────────┼──────────────┐
                          v              v              v
                       uint16          float          uint8
                       member          member         member
                          ...
                          └──── local typed decode ─────┘
                                         │
                                         v
                            Address | Symbol | Type | Value
```

Raw and Semantic are presentation layers over the same target-memory service. Semantic rendering must never create per-Symbol probe reads.


## V0.6.13 — Semantic refresh presentation state

```text
Symbol navigation
  -> set anchor + one-shot focus flag
  -> bounded ReadMemEx
  -> build typed rows
  -> center destination once

Auto/Manual refresh
  -> same bounded ReadMemEx
  -> rebuild/update typed values
  -> restore user vertical/horizontal viewport
  -> preserve only user-created selection
  -> never scroll back to anchor
```

The navigation anchor belongs to data-window planning. Scroll/current-row belong to presentation state and are not owned by refresh.


## V0.6.14 — Semantic Symbol navigation selection
Explicit Symbol navigation is a one-shot presentation event on top of the existing bounded semantic cache: resolve address, rebuild/consume local rows, center the matching row and select it. Later refreshes preserve viewport and actual selection only; `_anchor_address` never becomes a scroll owner. Empty-viewport click clears both selection and current index so Qt cannot resurrect a stale current row on model rebuild. No target I/O changes.


## V0.6.15 — Cross-module Symbol routing

The UI now exposes a small local routing layer:
...
The payload is only `(name, address, type_name)`. `MainWindow` connects these signals to the already-existing Watch/Scope/Memory instances. There is still exactly one `JLinkWorker`; cross-module actions never instantiate a backend or perform DLL access. `Open in Memory` validates exact Symbol full-name + address before choosing Semantic navigation; otherwise it uses raw-address navigation. RTT channels are explicitly excluded from memory/watch routing because their target-defined stream fields do not necessarily map to AXF addresses.
