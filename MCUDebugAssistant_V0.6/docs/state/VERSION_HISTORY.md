...
- V0.6.13 — Stable Semantic Refresh: Symbol navigation centers once; subsequent Auto Refresh preserves Symbols viewport and does not re-force an anchor selection.

- V0.6.12 — Semantic Memory View: Auto/Raw/Symbols modes; nearby AXF/DWARF scalar members are decoded by their own types from one bounded shared raw block.

- V0.6.11 — Engineering Presets: editable common-value drop-downs for Memory refresh, Watch polling, Scope sampling/buffer/FPS while preserving custom values and existing integer settings.

- V0.6.10 — Compact Connection Panel: auto-collapsible J-Link configuration with persistent status/Disconnect, plus de-duplicated Test Automation row numbering.

- V0.6.9 — Stable History Controls: adds persistent single-entry delete button and keeps existing history order stable when selecting saved Symbol/address entries.

- V0.6.8 — Memory Symbol/History Reliability: gates history activation to its own popup, makes QCompleter the single Symbol commit path, adds per-item history deletion, and migrates legacy Symbol width state to auto-fit.

- V0.6.7 — Memory Navigation + Reliable Watch Commit: exact keyboard/history Symbol selection, Symbol-first editor display, automatic Symbol width fitting, and same-value Watch focus-out writes via delegate commit.
